...
function ImgSlider({ url, limit = 5, page = 1 }) {
    ...
    function handelValueIncrement() {
        ...
        if (tempVal > images.length - 1) {
            setCurrentSilder(0)
        } else { setCurrentSilder(tempVal) }
    }

    async function fetchImges(getUrl) {
        console.log(`getUrl : ${getUrl}`);
        console.log(`fetch : ${getUrl}?page=${page}&limit=${limit}`);
        try {
            setLoading(true);
    ...
    console.log(`current Silder Value Index : ${currentSilder}`);
    console.log(images);


    return <div className="bg-violet-600 h-screen w-full grid place-content-center">
                ...
                images.map((img) => (
                    <div key={img.id}> {
                        currentSilder == parseInt(img.id) ?
                            <div>
                                <h1 className="text-white font-bold text-2xl text-center mt-4">Image Index : {parseInt(img.id) + 1}</h1>
                                <img key={img.id} src={img.download_url} alt={img.author} className="w-52 rounded-md mt-5" />
                            </div>
                            : null}
                    </div>
                ))
        ...
        <div className=" w-full flex justify-center text-white my-5">
            <FaArrowAltCircleLeft onClick={handelValuedecrement} size={40} className="mx-4" />
            <FaArrowAltCircleRight onClick={handelValueIncrement} size={40} className="mx-4" />
        </div>
    </div>;
}
